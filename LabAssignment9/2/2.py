...
def exp(rv):
    t, u = l2norm(rv), normalized(rv)
    c, s = np.cos(t), np.sin(t)
    return np.array([
        [c + (1-c) * u[0] ** 2, u[0]*u[1]*(1-c) -
         u[2]*s, u[0]*u[2]*(1-c)+u[1]*s],
        [u[1] * u[0] * (1-c) + u[2] * s, c + (1-c) *
         u[1]**2, u[1]*u[2]*(1-c)-u[0]*s],
        [u[2] * u[0] * (1-c) - u[1] * s, u[2]*u[1] *
         (1-c)+u[0]*s, c+(1-c)*u[2]**2],
    ])
...
def log(R):
    ...
    if tr == 3:
        ...
        if R[0,0] != -1:
            return np.pi * normalized(np.array([1+R[0,0], R[1,0], R[2,0]]) / np.sqrt(2 * (1+R[0,0])))
        elif R[1,1] != -1:
            return np.pi * normalized(np.array([R[0,1], 1+R[1,1], R[2,1]]) / np.sqrt(2 * (1+R[1,1])))
        elif R[2,2] != -1:
            return np.pi * normalized(np.array([R[0,2], R[1,2], 1+R[2,2]]) / np.sqrt(2 * (1+R[2,2])))
    theta = np.arccos((tr - 1) / 2)
    return theta * normalized(np.array([R[2,1]-R[1,2], R[0,2]-R[2,0], R[1,0]-R[0,1]]) / np.sin(theta) / 2)
...
def render(ang):
    global gCamAng, gCamHeight, gVisibles
...
def key_callback(window, key, scancode, action, mods):
    global gCamAng, gCamHeight, gMode
